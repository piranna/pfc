...
esto se solucione, se podra usar NodeOS en sistemas con una arquitectura de CPU
distinta de x86 como es el caso del micro-ordenador Raspberry Pi.

### Descarga de dependencias

Los distintos componentes de NodeOS son automaticamente descargados y compilados
desde internet, no asi las herramientas necesarias para su generación usandose
...
ser descargadas e instaladas automaticamente para facilitar la generación de
...
aunque esto implicaria un aumento considerable en el tiempo empleado en la
...
virtuales, drivers de tarjetas graficas...) y orientada principalmente para
...
mas proxima a la de un equipo de escritorio, lo cual permitiría usar el sistema
en estaciónes de trabajo y ofreceria un entorno óptimo para poder desarrollar
...
navegadores web Firefox y Chrome. Sin embargo, ésto último necesitaría un mejor
...
no estan en los paquetes instalados por defecto sin una razon aparente) tanto en
...
disponibles requieren del uso del entorno gráfico X11 o de sus librerias de una
...
pero también hay otros mas avazados como [OS.js](http://os.js.org), el cual ha
sido adaptado explicitamente por su autor para NodeOS para que pudiera funcionar
en servidores web estáticos y sin estado acorde a la arquitectura recomendada
para las aplicaciónes gráficas.
...
automaticamente puesto que initramfs es de hecho una cache de disco sin un
...
* [mover build-ins fuera del binario de Node.js](https://github.com/NodeOS/NodeOS/issues/170#issuecomment-141584801)
  ...
  lo cual ademas facilitaria la lectura del código de estos durante su ejecución.
  Hay propuestas similares dentro de la comunidad de Node.js para que se
  publiquen los build-ins como módulos independientes en `npm`, por lo que la
  idea podria tener acogida dentro del código principal en una futura versión.
