...
imagen minima de dichos sistemas operativos (como por ejemplo Ubuntu Core o
CoreOS, este ultimo diseñado especificamente para ser ejecutado en servidores en
la nube dentro de instancias Docker) y añadirle la maquina virtual de Node.js
para tener un entorno similar al ofrecido por NodeOS. Este enfoque es el que
propone el sistema operativo [Linino](http://www.linino.org), el cual esta
basado en [OpenWRT](https://openwrt.org) y que esta principalmente orientado
para su uso junto con micrcontroladores en robotica y en el Internet de las
Cosas.

Sin embargo, este enfoque de usar un sistema operativo ya existente (aunque sea
una version reducida) en lugar de ejecutar Node.js directamente sobre el kernel
...
uso de BareMetal OS como kernel ya que esta enfocado a ser usado en sistemas
...
requeriria previamente portar Node.js o la maquina virtual a usar a dicho
sistema.
...
* [ES Operating System](https://code.google.com/p/es-operating-system), proyecto
  ...
  capa en C++ con los drivers basicos (teclado, VGA) y sobre la que se ejecuta
  directamente un interprete Javascript y un motor de renderizado HTML (hecho
  tambien en Javascript), los cuales tienen acceso completo al sistema. Su mayor
  caracteristica es que todas las APIs del sistema estan auto-generadas a partir
  de especificaciones en formato [WebIDL](http://heycam.github.io/webidl) lo que
...
* [AnarchyOS](https://github.com/juliangruber/anarchyos), iniciado por algunos
  ...
  gestor de dependencias al estilo de [apt](https://wiki.debian.org/es/Apt) en
  los sistemas basados en [Debian](https://debian.org)/[Ubuntu](http://ubuntu.com)
  o [rpm](http://www.rpm.org)/[yum](http://yum.baseurl.org) en los sistemas
...
* [Runtime.js](http://runtimejs.org), el cual es un caso especial porque no solo
  ...
  hecho en torno al motor Javascript [v8](https://code.google.com/p/v8) (el
  ...
  permite aumentar el rendimiento usando un unico contexto de ejecucion e
...
[ChromeOS](https://es.wikipedia.org/wiki/Chrome_OS) y
[FirefoxOS](https://www.mozilla.org/es-ES/firefox/os/2.0/),
...
ejecutar localmente un motor de renderizado web como por ejemplo
[content shell](https://www.chromium.org/developers/content-module)
basado en Chromium o [Gecko](https://developer.mozilla.org/es/docs/Gecko) usado
como base de Firefox, por lo que NodeOS se convertiria en una alternativa a
...
dichas aplicaciones remotamente desde cualquier navegador web.
...
dedicado directamente sobre el, como demostraron [sus demos en la PyCon]() al
hacerse la presentacion directamente en este entorno gracias a su aceso a la
tarjeta de video. Ademas proximamente tambien podra usar los protocolos EFI para
tener acceso a la tarjeta de red, por lo que seria posible construir un servidor
web directamente en el.
...
scripts de administracion y el desarrollo de aplicaciones propias (por ejemplo,
[Ubuntu Software Center](https://wiki.ubuntu.com/SoftwareCenter) esta hecho
integramente en Python, incluido el desarrollo de nuevos widgets graficos
exclusivos de la aplicacion) o en el caso de *OLPC* por desarrollar bindings a
aplicaciones ya existentes (como es el caso del juego
[Micropolis](http://www.donhopkins.com/home/micropolis), hecho a partir del
codigo fuente de [Sim City](https://en.wikipedia.org/wiki/SimCity#SimCity)
...
es [Phantom](http://dz.ru/en/solutions/phantom), el cual hace uso de una maquina
...
dispositivos y demas usando en su lugar un pool de objetos persistentes que
ocupa todo el espacio de direcciones disponible, simplificando el diseño tanto
del sistema operativo como de sus aplicaciones al estar virtualmente todo
alojado en memoria en forma de objetos. Sistemas operativos como este pueden
tener una gran repercusion con nuevas arquitecturas de computadoras como la
propuesta por [HP](http://www.hp.com) con su proyecto
...
de la maquina virtual Java desarrollada por Sun resultando en una implementacion
hardware del lenguaje. Se planteo usarse como coprocesador en las
[JavaStation](https://en.wikipedia.org/wiki/JavaStation) para acelerar la
ejecucion de las aplicaciones de JavaOS, aunque dicha plataforma dejo de tener
...
con los procesadores Java y del cual los sistemas operativos web y muy tambien
especialmente NodeOS podrian sacar ventaja aumentando drasticamente su
rendimiento.
